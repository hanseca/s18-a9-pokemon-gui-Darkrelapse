...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   *@throws PokemonException if name is empty. 
   **/
   public Pokemon(String species, String name, 
      ...
      //Changed from 40 to 39 to fix out of bounds
      this.level = ((double) randGen.nextInt(39)) + 1;
   ...
   */
   private void calculateHPAndCP() {
      ...
      //Added to supplement a simple fix and is consistent for the test.
      if (cP < 10) {
         cP = 10;
      }
   }
   ...
   */
   ...
   /**
   * Makes equals method for pokemon.
   * @param p1 for pokemon used to check
   * @return boolean to check if same or not
   **/
   public boolean equals(Pokemon p1) {
      
      if (this.number == p1.number) {
         return true;
      }
   ...
   /**
   * Compares to another pokemon object.
   * @param p2 for pokemon being compared
   * @return tempNum for CompareTo number
   **/
   public int compareTo(Pokemon p2) {
      ...
      while (tempL) {
         ...
         if (this.equals(p2)) {           
            tempNum = 0;
            tempL = false;
         } else if (this.number != p2.number) {   
            if (this.number > p2.number) {
               tempNum = 1;
               tempL = false;
            } else {
               tempNum = -1;
